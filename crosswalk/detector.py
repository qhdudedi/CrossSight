"""[1단계] 객체 검출 + 추적  -  부분 모델 1: YOLO11 (COCO 사전학습) + ByteTrack
...
가중치
    GPU/MPS : yolo11s.pt   CPU : yolo11n.pt   (계획서: 정확도 vs 속도 트레이드오프)
...
ByteTrack
    ...
    ultralytics 의 model.track(persist=True) 안에 내장되어 있다.
...
class Detection:
    ...
    track_id: int  # ByteTrack 이 부여한 번호. 같은 사람/차량이면 프레임이 바뀌어도 같은 값
...
class Detector:
    ...
    def __call__(self, frame: np.ndarray) -> list[Detection]:
        """프레임 1장 -> Detection 목록"""
        # track(): 검출 + ByteTrack 추적을 한 번에 수행. persist=True 로 이전 프레임의 트랙 상태를 유지
        r = self.model.track(
            frame, persist=True, tracker="bytetrack.yaml", conf=self.conf, iou=0.5,
